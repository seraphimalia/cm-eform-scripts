...
function processPRFs (prfFormContainer, formType) {
  ...
  if (prfFormContainer.length > 0) {
    for (let i = 0; i < prfFormContainer.length; i++) {
      if (
        prfFormContainer.find(`label:contains('${numberField}')`)[i].nextSibling.value.length > 0 &&
        typeof prfFormContainer.find("label:contains('Triage')")[i] !== 'undefined'
      ) {
        const prf = {}
        if (formType === 'RHT' || formType === 'PRF') {
          prf[GOOGLE_FORM.PRF_NUMBER] =
            (formType === 'RHT' ? 'RHT' : '') +
            prfFormContainer.find(`label:contains('${numberField}')`)[i].nextSibling.value
        } else {
          ...
          prf[GOOGLE_FORM.METRO_REF] = prfFormContainer.find(`label:contains('${numberField}')`)[i].nextSibling.value
        }
        prf[GOOGLE_FORM.PATIENT_TRIAGE] = prfFormContainer.find("label:contains('Triage')")[i].nextSibling.value
        prf[GOOGLE_FORM.PATIENT_GENDER] = prfFormContainer.find("label:contains('Patient Gender')")[i].nextSibling.value
        prf[GOOGLE_FORM.PATIENT_AGE] = prfFormContainer.find("label:contains('Patient Age (Years)')")[
          i
        ].nextSibling.value
        prf[GOOGLE_FORM.PATIENT_ETHNIC_GROUP] = prfFormContainer.find("label:contains('Patient Ethnic Group')")[
          i
        ].nextSibling.value
...
function buildEForm () {
  ...
  if (sourceElementLength > 0) {
    ...
    vars[GOOGLE_FORM.CALL_SOURCE_OTHER] = $('#DynamicListsContainer')
      .find("div[data-groupid='319']")
      .find("label:contains('Call Source')")[indexSourceOther].nextSibling.innerText
  ...
  // Call Type
  vars[GOOGLE_FORM.CALL_TYPE] = extractCallTypeFromElement($('#PrimaryComplaintTitle'))
  ...
  if (firstOnSceneTimeline.length > 0) {
    const callsign = extractVehicleCallsignFromTimelineElement(firstOnSceneTimeline[firstOnSceneTimeline.length - 1])
    if (callsign) {
  ...
  const proceed = () => {
    ...
    for (let i = 0; i < prfs.length; i++) {
      ...
      const accountChooserURL =
        'https://www.google.com/accounts/AccountChooser?Email=&continue=' + encodeURIComponent(eFormURL)
  ...
  const whenNo = () => {
    ...
    const pagedTimeline = $('#timeline').find("div.panel:contains('Incident Paged Out')")
    if (pagedTimeline.length > 0) {
      vars[GOOGLE_FORM.PAGED_TIME] = extractTimeFromTimelineElement('paged', pagedTimeline[pagedTimeline.length - 1])
    }
    ...
    const onSceneTimeline = $('#timeline').find("div.panel:contains(' - On Scene')")
...
function findMetroReferenceNumber () {
  ...
  if (notes.length === 0) {
    notes = $('#NotesTable').find("div.row:contains('DoH - EMS (Metro) Ambulance Services')")
  }
  if (notes.length === 0) {
    notes = $('#timeline').find("div.panel:contains('DoH - EMS (Metro) Ambulance Services')")
  }
